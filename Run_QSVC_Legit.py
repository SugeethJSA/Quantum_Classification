...
def main():
    ...
    for run_idx in range(1, 11):
        ...
        K_train = compute_kernel_matrix(train_features_sub, train_features_sub, symmetric=True)
        
        print("Fitting SVC...")
        svc = SVC(kernel="precomputed", probability=True)
        svc.fit(K_train, train_labels_sub)
        
        model_path = os.path.join("saved_models", f"Hybrid_CNN_QSVC_Run{run_idx}.pkl")
        ...
        new_row = {"Model": f"Hybrid CNN-QSVC (Legit Run {run_idx})", "OOD_Accuracy": acc_q, "OOD_F1": f1_q, "OOD_Precision": prec_q, 
