...
results = {}
for m, name in [(resnet, "ResNet18"), (mobilenet, "MobileNetV3"), 
                (efficientnet, "EfficientNet-B0"), (legacy_cnn, "Custom_Legacy_CNN")]:
    results[name] = train_model(m, name, epochs=15)


print("\nTraining YOLOv8n-cls...")
yolo_model = YOLO('yolov8n-cls.pt')
yolo_res = yolo_model.train(data=SPLIT_DIR, epochs=15, imgsz=128, batch=32, device='0' if torch.cuda.is_available() else 'cpu', verbose=False, workers=0)
...
# ==============================================================================
# 1. Upgraded Quantum Circuit (Data Re-uploading & Amplitude Embedding)
# ==============================================================================
# n_qubits is 10, meaning AmplitudeEmbedding takes 2^10 = 1024 features
@qml.qnode(dev, interface="torch", diff_method="backprop")
def upgraded_quantum_circuit(inputs, weights):
    # Apply Amplitude Embedding ONCE
...
# ==============================================================================
# 2. Upgraded Hybrid CNN-QNN (From Scratch - now uses a better lightweight CNN)
# ==============================================================================
class HybridCNNQNN_Upgraded(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(128, 1024) # Project to 1024 for Amplitude Embedding
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        x = self.fc1(x)
        x = torch.nn.functional.normalize(x, p=2, dim=1, eps=1e-8) # L2 Norm for AmplitudeEmbedding
...
# ==============================================================================
# 3. Upgraded Custom Legacy QNN (Dense features to 1024)
# ==============================================================================
...
# ==============================================================================
# 4. Upgraded Hybrid CNN-QNN (Transfer Learning with EfficientNet-b0)
# ==============================================================================
class HybridCNNQNN_Transfer_Upgraded(nn.Module):
    def __init__(self):
        super(HybridCNNQNN_Transfer_Upgraded, self).__init__()
        efficientnet = models.efficientnet_b0(weights='DEFAULT')
        ...
        self.fc1 = nn.Linear(1280, 1024) # EfficientNet-b0 outputs 1280, project to 1024
        self.quantum = create_upgraded_quantum_layer()
        self.fc2 = nn.Linear(n_qubits, 2)
...
# ==============================================================================
# 5. EXECUTION: Train Upgraded models
# ==============================================================================
hybrid_model_upg = HybridCNNQNN_Upgraded().to(device)
legacy_qnn_upg = Custom_Legacy_QNN_Upgraded().to(device)
hybrid_tl_upg = HybridCNNQNN_Transfer_Upgraded().to(device)

models_to_run = [
    (hybrid_model_upg, "Hybrid CNN-QNN (From Scratch)"),
    (legacy_qnn_upg, "Custom_Legacy_QNN"),
    (hybrid_tl_upg, "Hybrid CNN-QNN (Transfer Learning)")
]

for m, name in models_to_run:
    results[name] = train_model(m, name, epochs=15)
    
print("\nTraining of Upgraded Hybrid models complete!")

# ==============================================================================
# 6. QSVC Benchmark (Phase 3)
# ==============================================================================
print("\n--- Running QSVC Benchmark ---")
from sklearn.svm import SVC
from sklearn.metrics import accuracy_score

# Extract features using the trained Hybrid Transfer Learning feature extractor
def extract_features_for_qsvc(dataloader, model):
    model.eval()
    all_features = []
    all_lbls = []
    with torch.no_grad():
...
# Use a small subset of training data for QSVC since kernel matrix is O(N^2)
train_features, train_labels_qsvc = extract_features_for_qsvc(train_loader, hybrid_tl_upg)
ninja_features, ninja_labels_qsvc = extract_features_for_qsvc(ninja_loader, hybrid_tl_upg)

# Limit training size for QSVC to avoid massive delays (e.g., 500 samples)
subset_size = min(500, len(train_features))
train_features_sub = train_features[:subset_size]
train_labels_sub = train_labels_qsvc[:subset_size]

# Define a simple quantum feature map for the kernel
...
print("Computing Quantum Kernel Matrix for QSVC... (this may take a moment)")
# We can use qml.kernels.kernel_matrix if available, or just compute it manually for the subset
import time
start_qsvc = time.time()
qkernel = qml.kernels.EmbeddingKernel(kernel_function, dev_kernel)

K_train = qkernel.square_kernel_matrix(train_features_sub)
svc = SVC(kernel="precomputed", probability=True)
svc.fit(K_train, train_labels_sub)

K_test = qkernel.kernel_matrix(ninja_features, train_features_sub)
qsvc_preds = svc.predict(K_test)
qsvc_probs = svc.predict_proba(K_test)[:, 1]
qsvc_time = time.time() - start_qsvc

results["Hybrid CNN-QSVC"] = {
    'name': "Hybrid CNN-QSVC",
    'preds': qsvc_preds,
    'labels': ninja_labels_qsvc,
    'probs': qsvc_probs,
    'fps': len(ninja_features) / qsvc_time, # approximate
    'params': hybrid_tl_upg.fc1.out_features, # classical embedding size
    'history': {'loss': [0], 'acc': [0]} # no epoch history
}
print("QSVC Benchmark complete!")
